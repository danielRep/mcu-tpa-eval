...
struct uart_pl011_dev_t UART1_PL011_DEV_NS =
{
    &(UART1_PL011_DEV_CFG_NS),
...
bool uart_init(void)
{
    ...
    uart_pl011_init(&UART1_PL011_DEV_NS, CPU_MHZ);
    uart_pl011_enable(&UART1_PL011_DEV_NS);

    if (uart_pl011_is_writable(&UART1_PL011_DEV_NS))
        return true;
...
int _read(int file, char *ptr, int len)
{
    ...
    if(ptr != NULL)
    {
        ...
        {
            while (!uart_pl011_is_readable(&UART1_PL011_DEV_NS)){};
            uart_pl011_read(&UART1_PL011_DEV_NS, &byte);
            *ptr = (char)byte;
            ++n_chars;
...
int _write(int file, const uint8_t *ptr, int len)
{
    if (isatty(file)) {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            while(!uart_pl011_is_writable(&UART1_PL011_DEV_NS)) {};

            (void)uart_pl011_write(&UART1_PL011_DEV_NS, buff[i]);

            if (buff[i] == '\n') {
                while(!uart_pl011_is_writable(&UART1_PL011_DEV_NS)) {};
                (void)uart_pl011_write(&UART1_PL011_DEV_NS, (uint8_t)'\r');
            }
        }
